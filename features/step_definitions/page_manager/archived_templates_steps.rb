...
When /^I delete the archived template$/ do
  ...
  template = @wildfire.page_manager.archived_pages_panel.get_template_by_title @template_name
...
Then /^there should be one less template in Archived$/ do
  @wildfire.page_manager.content_div.templates.count == @num_templates_before
end
